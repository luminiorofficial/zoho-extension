...
export default async function ProjectDetailPage({ params, searchParams }: PageProps<'/projects/[id]'>) {
  ...
  const content = (
    <>
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">Project</p>
          <div className="mt-1 flex flex-wrap items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900">{project.name}</h1>
            {!project.is_active && <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600">Archived</span>}
          </div>
        ...
        <ProjectDeleteAction projectId={project.id} projectName={project.name} isActive={project.is_active} />
      </div>
