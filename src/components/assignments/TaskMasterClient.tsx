...
export default function TaskMasterClient({ tasks }: { tasks: TaskMasterItem[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Task</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Status</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((task) => <tr key={task.id} className={task.isActive ? '' : 'bg-slate-50'}>{editingId === task.id ? <><td className="px-4 py-3"><input required value={title} maxLength={300} onChange={(event) => setTitle(event.target.value)} className={inputClass} /></td><td className="px-4 py-3"><input value={category} maxLength={100} onChange={(event) => setCategory(event.target.value)} placeholder="General" className={inputClass} /></td><td className="px-4 py-3 text-slate-500">{task.isActive ? 'Active' : 'Archived'}</td><td className="px-4 py-3"><div className="flex justify-end gap-2"><button type="button" disabled={pending} onClick={() => void request(`/api/task-master/${task.id}`, 'PATCH', { title, category })} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-50">Save</button><button type="button" onClick={() => setEditingId(undefined)} title="Cancel edit" className="rounded-lg border border-slate-300 p-2 text-slate-600"><X size={15} /></button></div></td></> : <><td className="px-4 py-3 font-medium text-slate-800">{task.title}</td><td className="px-4 py-3 text-slate-600">{task.category}</td><td className="px-4 py-3"><span className={`text-xs font-medium ${task.isActive ? 'text-emerald-700' : 'text-slate-500'}`}>{task.isActive ? 'Active' : 'Archived'}</span></td><td className="px-4 py-3"><div className="flex justify-end gap-2"><button type="button" onClick={() => { setEditingId(task.id); setTitle(task.title); setCategory(task.category); }} title="Edit task" className="rounded-lg border border-slate-300 p-2 text-slate-600 hover:bg-slate-50"><Pencil size={15} /></button><button type="button" disabled={pending} onClick={() => void request(`/api/task-master/${task.id}`, 'PATCH', { isActive: !task.isActive })} title={task.isActive ? 'Archive task' : 'Restore task'} className="rounded-lg border border-slate-300 p-2 text-slate-600 hover:bg-slate-50 disabled:opacity-50">{task.isActive ? <Archive size={15} /> : <RotateCcw size={15} />}</button></div></td></>}</tr>)}</tbody></table></div>
